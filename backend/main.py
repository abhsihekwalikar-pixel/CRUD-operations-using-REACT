# ==========================================
# USER MANAGEMENT SYSTEM - JWT + BEARER TOKEN
# FastAPI + SQLite + JWT + RBAC + Bcrypt
# ==========================================
...
def stats(db: Session = Depends(get_db)):
    ...
    recent_models = db.query(User).order_by(User.created_at.desc()).limit(5).all()
    recent_users = [
        {
            "id": user.id,
            "name": user.name,
...
@app.get("/api/users/recent", tags=["Public"])
def recent_users(limit: int = 5, db: Session = Depends(get_db)):
    recent_models = db.query(User).order_by(User.created_at.desc()).limit(limit).all()
    return [
        {
            "id": user.id,
            "name": user.name,
